...
import (
	...
	"github.com/skicean/pgx/v5/internal/iobufpool"
	"github.com/skicean/pgx/v5/internal/nbconn"
	"github.com/skicean/pgx/v5/internal/pgio"
	"github.com/skicean/pgx/v5/pgconn/internal/ctxwatch"
	...
	for {
		...
		case *pgproto3.ReadyForQuery:
			...
			if config.ValidateConnect != nil {
				...
				//
				// See https://github.com/skicean/pgconn/issues/40.
...
func (pgConn *PgConn) Close(ctx context.Context) error {
	...
	if ctx != context.Background() {
		...
		//
		// See https://github.com/skicean/pgconn/issues/29
		pgConn.contextWatcher.Unwatch()
	...
	// ignores errors.
	//
	// See https://github.com/skicean/pgx/issues/637
...
func (pgConn *PgConn) BindExec(ctx context.Context, stmtName string, paramValues [][]byte, paramFormats []int16, resultFormats []int16) *ResultReader {
	...
	pgConn.frontend.SendBindExec(&pgproto3.BindExec{PreparedStatement: stmtName, ParameterFormatCodes: paramFormats, Parameters: paramValues, ResultFormatCodes: resultFormats})
	err := pgConn.frontend.Flush()
	if err != nil {
		pgConn.asyncClose()
